...
const CardPrice:React.FC<CardPriceProps> = ({data, currency, btn}) => {
    ...
    return (
        <div className="relative w-[90%] max-md:w-full h-full flex items-center">
            {canScrollLeft && (
                <button 
            ...
            <div ref={scrollRef} className="flex gap-4 overflow-hidden w-full">
                {data.map(n => (
                    <div 
                    key={n.id}
                    className={`cursor-pointer border border-lightGray rounded-xl py-4 px-6 h-[420px] w-80 min-w-[330px] max-md:w-full flex flex-col justify-around hover:shadow ${n.type === "Basic" ? "bg-lightGray shadow" : "bg-dakBlue"}`}
                >
                    <div>
                        <h2
