        ...
        <div className="relative w-[80%] max-md:w-[90%] h-full flex items-center">
            ...
            <div ref={scrollRef} className="flex gap-4 overflow-hidden w-full">
                {data.map(n => (
                    <div 
                    key={n.id}
                    className={`cursor-pointer border border-lightGray rounded-xl py-4 px-6 h-[420px] w-80 min-w-[330px] max-mf:w-full flex flex-col justify-around hover:shadow ${n.type === "Basic" ? "bg-darkBlue shadow" : "bg-lightGray"}`}
                >
                    <div>
                        <h2
                            className={`text-3xl font-bold ${n.type === "Basic" ? "text-white":"text-darkBlue"}`}
                        >{n.type}</h2>
                        <p
                            className={`${n.type === "Basic" ? "text-lightGray":"text-black"} font-light text-md leading-[20px] mt-2`}
                        >{n.txt}</p>
                    </div>
                    <div
                        className={`flex ${n.type === "Basic" ? "text-white":"text-darkBlue"} font-bold`}
                    >
                        <h3 
                            className="text-5xl"
                        >
                    ...
                    <ul>
                        {
                            n.ben.map((i)=> (
                                <li key={i} 
                                    className={`flex items-start ${n.type === "Basic" ? "text-lightGray":"text-black"} `}
                                > 
                                    <div 
                                        className={`${n.type === "Basic" ? "bg-white":"bg-darkBlue"} p-[2px] text-center rounded-full text-white mr-2`}>
                                    <svg 
                                    className={`flex-shrink-0 size-4 ${n.type === "Basic" ? "text-darkBlue":"text-white"}`}
                                    fill="currentColor"
                    ...
                    <Link href={btn.slug}
                        className={`outline-none ${n.type === "Basic" ? "text-darkBlue bg-white hover:bg-lightGray ":"text-white bg-darkBlue hover:bg-skyBlue"} font-semibold focus:ring-4 focus:ring-purple-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center outline-none`}
                        >{btn.name}</Link>
                </div>
                ))}
            </div>
